...
当前已实测 YouTube、B 站、抖音和小红书公开单视频。抖音与小红书可直接粘贴分享文本或短链；小红书若跳转登录页，可在解析框临时填写网页版 Cookie。视频号普通分享链接需要临时填写元宝登录 Cookie；已包含 `token`、`eid` 的播放页链接可以直接解析，但真实作品下载与解密还需要进一步验证。其他平台、登录限制、分 P、直播和地址过期等覆盖范围见 [视频解析说明](video-parser-integration.zh-CN.md)。解析结果超过 15 分钟或服务重启后，需要在添加任务前重新解析；已经添加的任务会保存自己的媒体计划。
