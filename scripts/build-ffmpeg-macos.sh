#!/bin/sh
# A local-file-only remux helper. No networking, encoders, or third-party codecs.
set -eu
...
FFDM_BUILD="$FFDM_CACHE/build-$FFDM_VERSION-$FFDM_ARCH"
FFDM_RESOURCES="$FFDM_ROOT/src-tauri/resources"
test "$(uname -s)" = Darwin || { echo 'This helper builds FFmpeg for macOS.' >&2; exit 1; }
...
test "$(shasum -a 256 "$FFDM_ARCHIVE" | cut -d ' ' -f 1)" = "$FFDM_SHA256" || {
...
if [ ! -f ffmpeg ]; then
  "$FFDM_SOURCE/configure" \
    --cc=/usr/bin/clang --disable-autodetect --disable-everything \
    ...
    --extra-ldflags=-mmacosx-version-min=13.0 > configure.log 2>&1
  make -j "${FFDM_BUILD_JOBS:-6}" > make.log 2>&1
fi
install -m 755 ffmpeg "$FFDM_RESOURCES/ffmpeg"
cp "$FFDM_SOURCE/COPYING.LGPLv2.1" "$FFDM_RESOURCES/third-party/FFmpeg-LICENSE.txt"
cp "$FFDM_ARCHIVE" "$FFDM_RESOURCES/third-party/"
cp "$FFDM_ROOT/scripts/build-ffmpeg-macos.sh" "$FFDM_RESOURCES/third-party/"
"$FFDM_RESOURCES/ffmpeg" -version | head -3
